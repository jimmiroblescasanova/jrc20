<div class="grid lg:grid-cols-2 gap-12 items-center">
    <div class="mt-12 lg:mt-0 lg:text-left text-gray-800 text-center">
        ...
        <p class="text-gray-600">
            Suscribete a nuestro email de noticias, para recibir información sobre nuevos eventos ya sea en
            línea o presencial.
    ...
    <div class="mb-12 lg:mb-0">
        <div class="block rounded-lg shadow-lg bg-white px-6 py-12 md:px-12">
            @if (!session('suscribed'))
                <form action="{{ route('guest.events.suscription') }}" method="POST">
                    ...
                    <input type="text" name="company"
                        ...
                        placeholder="Empresa" />
                    <div class="form-check justify-center mb-6">
                        <input type="checkbox"
                            class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                            value="1" name="accepted" id="flexCheckChecked">
                        <label class="form-check-label inline-block text-gray-800" for="flexCheckChecked">
                            Subscribirme a las noticias
                        </label>
                        @error('accepted')
                            <small class="block text-xs text-red-700">{{ $message }}</small>
                        @enderror
                    </div>
            ...
            @else
                <svg class="mb-2 w-10 h-10 text-gray-500" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"
                ...
                <a href="#">
                    <h5 class="mb-2 text-2xl font-semibold tracking-tight text-gray-900">¡Gracias por
                        suscribirte!</h5>
                </a>
                <p class="mb-3 font-normal text-gray-500">
                    {{ session('suscribed') }}, muy pronto recibirás en tu correo una confirmación del
                    registro y cuando haya algún evento nuevo, se te informará por el mismo medio.
                </p>
            @endif
        </div>
    </div>
